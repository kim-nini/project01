...
let sumPrice;
itemPrice.forEach(function (itemPriceElement) {
    var priceData = itemPriceElement.getAttribute('data-item');
    console.log(priceData);
    sumPrice = Number(priceData);
    sumPrice += sumPrice;
    console.log("합계 : "+ sumPrice);
});
// document.querySelector('.item_price').innerHTML = sumPrice;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.plus_btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
            var numberInput = btn.parentElement.querySelector('.number');
            var itemPrice = btn.parentElement.parentElement.querySelector('#itemPrice').value;
            var totalPriceElement = btn.parentElement.parentElement.querySelector('.price_amount');
            var totalAmountElement = btn.parentElement.parentElement.parentElement.querySelector('.num');

            var currentAmount = parseInt(numberInput.value);
            var newAmount = currentAmount + 1;

            // 업데이트된 수량 적용
            numberInput.value = newAmount;

            // 상품당 총 가격 업데이트
            var totalAmount = newAmount * parseInt(itemPrice);
            totalPriceElement.innerText = totalAmount + '원';
    ...
    function updateTotal() {
        ...
        totalElement.innerText = total + '원';
    }
...
// //===========================================================================================================================
//
// // 상품 1개당 + ,  - 버튼 눌러서  상품당 총 가격 계산하기
//
// $(".minus_btn").on("click", function() {
//     var now = $(".number").val();
//     if ($.isNumeric(now) && parseInt(now) > 0) {
//         now--;
//         $(".number").val(now).trigger("input");
//     }
// });
//
